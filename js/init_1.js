...
$(function () { // DOM ready
    ...
    $('.icon').live('touchstart mousedown', function (e) {
        ...
        if (!placement_in_course && !placement_not_allowed) {
            dragmove = false;
            $icon = $(this);
            $icon.css('z-index', 20);
            ...
            if (Math.abs(distance_x) > Math.abs(distance_y)) {
                if (distance_x > CUT_ICON / 2) {
                    ...
                    if (icon_col < NB_COLUMNS - 1) {
                        ...
                        placement(icon_line, icon_col, icon_line, icon_col + 1);
                    }
                }
                ...
                if (distance_y > CUT_ICON / 2) {
                    ...
                    if (icon_line < NB_LINES - 1) {
                        dragmove = true;
                        $('.icon').removeClass('click adjacent');
                        placement(icon_line, icon_col, icon_line + 1, icon_col);
                    }
                }
    ...
    $('#zone_play').live('touchend mouseup', function (e) {
        if (placement_in_course) {
            placement_in_course = false;
            $icon.css('z-index', 10);
            if (!dragmove) {
                verif_click($icon);
            }
...
function placement(icon_line, icon_col, binomial_line, binomial_col) {
    placement_in_course = false;
    placement_not_allowed = true;

    clearTimeout(hint_timeout);
    $('.hint').removeClass('hint');
    hint_mode = false;

    $binomial = $('.icon[data-line=' + binomial_line + '][data-col=' + binomial_col + ']');
    ...
    var binomial_num_origin = tab_icons[binomial_line][binomial_col];
    ...
    $('#zone_play').find('.icon').each(function () {
        ...
        if ($(this).attr('data-line') == icon_line_origin) {
            console.log("CUT_ICON " + CUT_ICON+" "+eval(parseInt($(this).css('left')) * CUT_ICON));
...
//    $icon.css({
//        'left': binomial_col_origin * CUT_ICON,
//        'top': binomial_line_origin * CUT_ICON
//    });
//    $binomial.css({
//        'left': icon_col_origin * CUT_ICON,
//        'top': icon_line_origin * CUT_ICON
//    });

//    tab_icons[icon_line_origin][icon_col_origin] = binomial_num_origin;
//    tab_icons[binomial_line_origin][binomial_col_origin] = icon_num_origin;
//
...
//            $binomial.attr('data-line', binomial_line_origin);
//            $binomial.attr('data-col', binomial_col_origin);
//
//            $icon.css({
//                'left': icon_col_origin * CUT_ICON,
//                'top': icon_line_origin * CUT_ICON
...
//            tab_icons[binomial_line_origin][binomial_col_origin] = binomial_num_origin;
//
//            setTimeout(function () {
//                verif_board();
//            }, 300);
